...
$sql = "INSERT INTO penjualan (kode_produk, tgl_penjualan, jumlah, harga_jual, total_nilai, keterangan)
        VALUES (?, ?, ?, ?, ?, ?)";
$stmt = $koneksi->prepare($sql);

try {
    $stmt->execute([$kode_produk, $tgl_penjualan, $jumlah, $harga_jual, $total_nilai, $keterangan ?: null]);
    header('Location: TampilPenjualan.php?success=1');
} catch (\PDOException $e) {
    error_log("Error insert penjualan: " . $e->getMessage());
